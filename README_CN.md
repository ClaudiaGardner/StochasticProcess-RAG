...
### 2. 多文档支持
- 自动解析 `data/` 目录下所有 PDF（教材、习题、试卷、讲义等）
- 支持追加模式，保留现有数据

### 3. 三种运行模式
- **标准模式**：使用在线 API（Claude/GPT）生成答案
- **离线模式**：使用本地 Ollama 模型，完全不联网
- **纯检索模式**：🎓 **考试推荐**！直接返回教材原文，不依赖 AI
...
**适用场景**：考试时需要快速查找教材原文，不需要 AI 生成答案。

```bash
# 纯检索模式：只返回教材原文和已有解答，不使用 LLM
python main.py --search
```

**优势**：
- ✅ 返回**精准的教材原文**，不会有 AI "编造"的内容
- ✅ **启动更快**，不需要加载 LLM 模型
- ✅ **完全离线**，不需要 Ollama 或任何 API
- ✅ 直接显示已解答的例题/习题的完整解答
